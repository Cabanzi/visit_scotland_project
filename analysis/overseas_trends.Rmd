...
mode_of_travel_and_purpose %>%
  ...
  ggplot(aes(x = year,
             ...
             fill = mode_of_travel)) +
  geom_col() +
  geom_text(aes(label = number_of_visits),
            position = position_stack(vjust = 0.5),
            size = 3) +  # Add annotations with values
  labs(title = "Total Number of Visits by Mode of Transport",
       x = "Year",
       y = "Number of Visits") +
  theme_minimal() +
...
```{r}
visits_to_uk_regions_by_qtr_2018_22_numeric %>% 
  filter(region_visited == "Scotland",
         year != c("2020 Qtr 1"| "2020 Qtr 2"| "2020 Qtr 3"| "2020 Qtr 4")) %>% 
